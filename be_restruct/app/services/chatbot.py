...
async def get_respone(
    ...
    chatbot_conf = settings.LLM_ENDPOINT
    ...
    startTime = time.time()
    start_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(startTime))
    ...
    endTime= time.time()
    end_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(endTime))
